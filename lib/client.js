 ...
 */
Client.prototype.flushConfigure = function (matcher) {
  var command = constants.ADMIN_PROMPT + constants.EXPORT_CONFIG_CMD;
  return new Promise((resolve, reject) => {
    this.sendAdminCommand2(command, matcher, null, null, null,
                         Long.ZERO, Long.NEG_ONE, -1, callbackWrap(resolve, reject));
  });
};
 ...
 * Reset the snapshot.
 */
Client.prototype.resetSnapshot = function () {
 ...
 */
Client.prototype.getSnapshot = function (type, matcher, selector, orderBy) {
  ...
  return new Promise((resolve, reject) => {
    this.sendAdminCommand2(command, matcher, selector, orderBy, null,
                         Long.ZERO, Long.NEG_ONE, -1,
      function (err, response) {
        if (!err) {
          resolve(new Cursor(response, that));
          return;
        }
        if (err.flags === constants.SDB_DMS_EOC) {
          resolve(null);
